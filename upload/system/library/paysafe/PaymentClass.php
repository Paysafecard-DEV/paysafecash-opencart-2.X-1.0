 ...
 */
    public function initiatePayment($amount, $currency, $customer_id, $customer_ip, $success_url, $failure_url, $notification_url, $webhook_url, $customer_data = array(), $time_limit = 4320, $correlation_id = "", $country_restriction = "", $kyc_restriction = "", $min_age = "", $shop_id = "", $submerchant_id = "")
    {
        ...
        if ($correlation_id != "") {
            //echo '<pre>Correlation: '.print_r($correlation_id,true).'</pre>';
            //exec( 'echo Correlation: "'.print_r($correlation_id, true). '" >> /tmp/presta.log');
            $headers = ["Correlation-ID: " . $correlation_id];
        } else {
            $headers = [];
        }
        //echo '<pre>json: '.print_r($jsonarray,true).'</pre>';
